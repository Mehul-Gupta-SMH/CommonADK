...
_REGISTRY = {
    ...
    # ... and so on as the remaining targets in _UNPORTED_TARGETS get a runner.
}
_UNPORTED_TARGETS = {"autogen", "langgraph"}  # shrinks to {} as each gets a runner
```
...
**All six targets now have a runner** (issue #22), so
`runners/__init__.py`'s `_UNPORTED_TARGETS` is empty. It is kept on purpose:
a new adapter (issue #11) can land before its runner does, and being listed
there is what earns that target the specific "buildable, but not yet
traceable" error instead of a misleading "unknown target".

The rows below are kept for history -- this is what the plan looked like
*before* implementation -- and are marked **IMPLEMENTED**. For any target,
**read the runner module's own docstring first**: that is the authoritative,
verified mapping. This table is the prior hypothesis, and implementing it
corrected several of its claims; each row notes where. AutoGen and LangGraph
have their own dedicated sections below (after "OpenAI Agents SDK"), written
after implementation and verified against real offline fake-model-driven
runs, so their speculative planning rows have been removed rather than left
here to contradict them.

| Target | Native run surface (file : line) | LLMCall usage source | Cost | Tool-call source | Transfer source | Session/multi-turn story |
|---|---|---|---|---|---|---|
| **Claude Agent SDK -- IMPLEMENTED, see `runners/claude_agent.py`** (`claude-agent-sdk` 0.2.144) | `claude_agent_sdk.query(prompt=, options=)`, an `AsyncIterator[Message]` where `Message = UserMessage \| AssistantMessage \| SystemMessage \| ResultMessage` (`claude_agent_sdk/types.py:1477`). `AssistantMessage.content` carries `TextBlock`/`ThinkingBlock`/`ToolUseBlock` (`:935-957`); a following `UserMessage` carries the matching `ToolResultBlock` (`.tool_use_id`, `.is_error`, `:959`). **Implemented via `ClaudeSDKClient.query()`/`.receive_response()`, not the one-shot `query()` function** -- see below. | `ResultMessage.usage: dict[str, Any]` and, more precisely, `ResultMessage.model_usage: dict[str, ModelUsage]` (`:1293`) — **per-model** breakdown (`inputTokens`, `outputTokens`, `cacheReadInputTokens`, `cacheCreationInputTokens`) — but only at the **end of the whole turn**, not per individual model call the way ADK/OpenAI Agents report it; a turn with several internal model calls (tool-calling loop) still yields exactly one `ResultMessage`. So `LLMCall` here is coarser-grained by construction: **one `LLMCall` per `run()` call**, not one per underlying model round-trip — a real, SDK-imposed limit stated plainly rather than fabricating finer granularity. **Correction found during implementation**: `AssistantMessage.usage` (`message_parser.py:215`) IS populated per-message, finer than this row originally claimed -- deliberately still not used, to keep token/cost granularity consistent; see `claude_agent.py`'s module docstring, point 1. | **Uniquely, the SDK computes cost itself**: `ResultMessage.total_cost_usd: float \| None` — the pricing table in `pricing.py` is **bypassed entirely** for this runner; `LLMCall.cost_usd` comes straight from the CLI's own computation. | `ToolUseBlock`/`ToolResultBlock` pairing, matched by `.id`/`.tool_use_id`; `duration_ms` is self-timed for individual tool calls (same limitation as the other two shipped runners) but the one coarse `LLMCall` gets a genuinely SDK-reported `duration_ms` from `ResultMessage.duration_api_ms` -- better than either other shipped runner can do. | Subagent invocation is a specific tool call (the `"Agent"` tool). **Correction found during implementation**: `AssistantMessage.parent_tool_use_id` correlating back to that tool call's id (verified in `message_parser.py`) *is* a real, usable SDK signal for both `Transfer` and `AgentStarted`/`AgentFinished` subagent boundaries -- not a guess, contrary to this row's original hedge. | `ClaudeSDKClient` keeps a persistent connection across `.query()` calls -- implemented exactly as this row originally proposed: the open client is cached in `RunSession.native["claude"]["client"]` across turns; an ad hoc (`session=None`) call always disconnects its own client. |
| **CrewAI -- IMPLEMENTED, see `runners/crewai_runner.py`** (`crewai` 1.15.16) | `crew.kickoff()` (sync) / `crew.kickoff_async()` — no native async event stream from `kickoff` itself, but **implemented via `crewai.events.crewai_event_bus`**, CrewAI's own internal pub/sub bus every subsystem already publishes onto (not investigated in this row originally) -- see `crewai_runner.py`'s module docstring for the full correction and the real limitation this approach has (cross-event-kind ordering is not guaranteed; see that docstring's "THE THREAD-CROSSING CAVEAT"). | **Correction found during implementation**: this row's claim that CrewAI has "no per-agent or per-call breakdown" was wrong -- `LLMCallCompletedEvent` (`crewai/events/types/llm_events.py`) carries per-call `usage`/`model`/`agent_role`, so the shipped runner emits one genuinely per-call, per-agent `LLMCall`, not the crew-wide singleton this row originally planned. `UsageMetrics`' fields are plain `int`s defaulting to `0` (not `Optional`), so `usage=None` or an all-zero result is conservatively treated as unreported -- the same 0-not-`None` reading `openai_agents.py` settled on. | No SDK-native cost; `pricing.py`, applied per call using the event's own `model`/tokens. | **Correction found during implementation**: this row flagged `step_callback`/`task_callback` as "the next thing to investigate" -- the shipped runner uses neither. `ToolUsageFinishedEvent`/`ToolUsageErrorEvent` (`crewai/events/types/tool_usage_events.py`) are the real native source, and `ToolUsageFinishedEvent` even carries genuine SDK-reported `started_at`/`finished_at` timestamps (not self-timed, unlike every other runner in this codebase). | **Correction found during implementation**: this row said delegation "has no observable event ... likely needs the same callback mechanism as tool calls" -- wrong. CrewAI's delegation IS an ordinary tool call (`"Delegate work to coworker"`/`"Ask question to coworker"`, `crewai/tools/agent_tools/`), fully visible on the same `ToolUsageFinishedEvent`, with a `coworker` key in `tool_args` naming the target directly. | No session support -- `RunSession` raises `NotImplementedError` immediately, exactly the fallback this row already anticipated ("may need to raise `NotImplementedError` per this doc's session contract"): CrewAI has no first-class "continue this exact conversation" primitive, and `crewai_adapter.py`'s `Crew` is built fresh with `tasks=[]` every call. |
...
- **Cross-SDK sessions.** A `RunSession` can hold state for more than one
  ...
  *build*, which routes each turn through one full agent graph on one SDK
  at a time already.
- **The still-unported runners** — mapped above, not built; see
  `runners.known_unported_targets()` for exactly which remain at any point
  in time (two of the original four, `claude` and `crewai`, are now
  implemented — see the corrected rows above).
